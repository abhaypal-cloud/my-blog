...
app.post('/api/posts', (req, res) => {
  ...
  posts.push(newPost);
  writePosts(posts) ? res.status(201).json(newPost) : res.status(500).json({ error: "Save error" });
});
...
app.put('/api/posts/:id', (req, res) => {
  ...
  posts[idx] = { ...posts[idx], title, content, readingTime, tags: tags || posts[idx].tags };

  writePosts(posts) ? res.json(posts[idx]) : res.status(500).json({ error: "Save error" });
});
...
app.delete('/api/posts/:id', (req, res) => {
  ...
  posts.splice(idx, 1);
  writePosts(posts) ? res.json({ success: true }) : res.status(500).json({ error: "Save error" });
});
